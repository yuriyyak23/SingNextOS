...
public sealed record ProtocolCancellationTransitionV1(uint MessageId, string FromState, string ToState);
...
public sealed class ProtocolDefinitionV1
{
    ...
    private readonly ProtocolCancellationTransitionV1[] _cancellationTransitions;
    private readonly Dictionary<(string State, uint MessageId), ProtocolCancellationTransitionV1> _cancellationTransitionsByKey;
    ...
    public ProtocolDefinitionV1(
        ...
        IEnumerable<ProtocolCancellationTransitionV1>? cancellationTransitions = null)
    {
        ...
        _cancellationTransitions = (cancellationTransitions ?? []).OrderBy(static transition => transition.FromState, StringComparer.Ordinal).ThenBy(static transition => transition.MessageId).ThenBy(static transition => transition.ToState, StringComparer.Ordinal).ToArray();
        _cancellationTransitionsByKey = [];
        foreach (var transition in _cancellationTransitions)
        {
            if (!_messagesById.ContainsKey(transition.MessageId)) throw new ArgumentException($"Cancellation transition references unknown message {transition.MessageId}.", nameof(cancellationTransitions));
            if (string.IsNullOrWhiteSpace(transition.FromState) || string.IsNullOrWhiteSpace(transition.ToState)) throw new ArgumentException("Cancellation transition states are required.", nameof(cancellationTransitions));
            if (!_cancellationTransitionsByKey.TryAdd((transition.FromState, transition.MessageId), transition)) throw new ArgumentException($"Duplicate cancellation transition for {transition.FromState}/{transition.MessageId}.", nameof(cancellationTransitions));
        }
    }
    ...
    public IReadOnlyList<ProtocolCancellationTransitionV1> CancellationTransitions => _cancellationTransitions;
    ...
    public bool TryCancellationTransition(string state, uint messageId, out ProtocolCancellationTransitionV1 transition) => _cancellationTransitionsByKey.TryGetValue((state, messageId), out transition!);
