...
public sealed class SingPlusGenerator : IIncrementalGenerator
{
    ...
    {
        if (!ValidatePayloadContract(output, contract)) return;
    ...
    private static bool ValidatePayloadContract(SourceProductionContext output, INamedTypeSymbol contract)
    {
        var valid = true;
        foreach (var method in contract.GetMembers().OfType<IMethodSymbol>().Where(static m => m.MethodKind == MethodKind.Ordinary))
        {
            ...
            foreach (var parameter in method.Parameters)
            {
                ...
                var implementsBoundedPayload = ImplementsBoundedPayload(parameter.Type);
                if (boundedAttribute is not null)
                {
                    boundedParameterCount++;
                    var maxBytes = BoundedPayloadMaxBytes(boundedAttribute);
                    if (maxBytes <= 0)
                    {
                        Report(output, BoundedPayloadDiagnostic, parameter, $"Bounded payload '{parameter.Name}' must declare a positive MaxBytes value.");
                        valid = false;
                    }
                    if (!implementsBoundedPayload)
                    {
                        Report(output, BoundedPayloadDiagnostic, parameter, $"Bounded payload '{parameter.Name}' must implement SingPlus.Contracts.IBoundedPayload.");
                        valid = false;
                    }
                    if (parameter.RefKind != RefKind.None)
                    {
                        Report(output, BoundedPayloadDiagnostic, parameter, $"Bounded payload '{parameter.Name}' cannot use ref, in, or out passing.");
                        valid = false;
                    }
                    if (parameter.Type is not INamedTypeSymbol namedBounded || !HasStableRuntimeTypeName(namedBounded))
                    {
    ...
    private static bool ImplementsBoundedPayload(ITypeSymbol type) =>
        type.AllInterfaces.Any(static i => i.Name == "IBoundedPayload" && i.ContainingNamespace.ToDisplayString() == "SingPlus.Contracts");
    ...
    private static string GenerateProtocol(ContractModel model)
    {
        ...
        b.AppendLine("        },");
        b.AppendLine("        new global::SingPlus.Contracts.ProtocolCancellationTransitionV1[]");
        b.AppendLine("        {");
        foreach (var transition in model.CancellationTransitions) b.Append("            new global::SingPlus.Contracts.ProtocolCancellationTransitionV1(").Append(transition.MessageId.ToString(CultureInfo.InvariantCulture)).Append("u, ").Append(Literal(transition.From)).Append(", ").Append(Literal(transition.To)).AppendLine("),");
        b.AppendLine("        });");
        b.AppendLine("}");
        return b.ToString();
    }
    ...
    private sealed class ContractModel
    {
        ...
        public static ContractModel Create(INamedTypeSymbol symbol)
        {
            ...
            var cancellationTransitions = messages.SelectMany(static m => m.CancellationTransitions).OrderBy(static t => t.From, StringComparer.Ordinal).ThenBy(static t => t.MessageId).ThenBy(static t => t.To, StringComparer.Ordinal).ToArray();
            var canonical = BuildCanonical(fullName, initial, terminals, messages, transitions, cancellationTransitions);
        ...
        private static string BuildCanonical(string fullName, string initial, IReadOnlyList<string> terminals, IReadOnlyList<MessageModel> messages, IReadOnlyList<TransitionModel> transitions, IReadOnlyList<TransitionModel> cancellationTransitions)
        {
    ...
    private sealed class MessageModel
    {
        ...
        public static MessageModel Create(IMethodSymbol method, string initial)
        {
            ...
            var cancellationTransitions = method.GetAttributes().Where(static a => a.AttributeClass?.Name == "CancellationTransitionAttribute").Select(a => new TransitionModel { MessageId = id, From = (string?)a.ConstructorArguments[0].Value ?? initial, To = (string?)a.ConstructorArguments[1].Value ?? initial }).ToArray();
