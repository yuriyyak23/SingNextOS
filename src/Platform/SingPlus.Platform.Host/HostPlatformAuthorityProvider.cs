    ...
    IPlatformVirtualizationProvider
{
        ...
        bool deferDsc1Completion = false)
    {
        ...
        featureDescriptors.Add(new PlatformFeatureDescriptor(
            ...
            PlatformFeatureAvailability.ModelOnly));
        featureDescriptors.Add(new PlatformFeatureDescriptor(
            PlatformFeatureFamily.GuestMemory,
            PlatformGuestMemoryContract.ContractVersion,
            PlatformFeatureAvailability.ModelOnly));
        featureDescriptors.Add(new PlatformFeatureDescriptor(
            PlatformFeatureFamily.VirtualEvents,
            PlatformVirtualEventContract.ContractVersion,
            PlatformFeatureAvailability.ModelOnly));
        ...
        _featureManifest = overrides is null
            ? new PlatformFeatureManifest(featureDescriptors)
            : new PlatformFeatureManifest(featureDescriptors
                .Where(defaultFeature => overrides.All(candidate => candidate.Family != defaultFeature.Family))
                .Concat(overrides));
    }
