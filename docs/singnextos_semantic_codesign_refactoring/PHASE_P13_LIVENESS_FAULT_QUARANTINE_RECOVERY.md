# P13 — Make safety/liveness/fault assumptions explicit and bound quarantine where possible

## 1. Goal
Make safety/liveness/fault assumptions explicit and bound quarantine where possible.

## 2. Rationale / live gap
- unknown->quarantine is safe but not progress.
- persistent/cold recovery remains partially excluded.
...
- **VERIFIED_EXISTING:** `src/Runtime/SingPlus.Runtime/VNext/ExternalOperationResourceBinding.cs`
- **VERIFIED_EXISTING:** `HybridCPU_ExternalRuntime/HybridCpuExternalRuntime.cs`
- **VERIFIED_EXISTING:** `src/Runtime/SingPlus.Runtime/Checkpointing/RuntimeKernel.Checkpointing.cs`

## 6. NEW_PROPOSED surfaces
- **NEW_PROPOSED:** `ProviderTrustClassV1`
- **NEW_PROPOSED:** `FaultModelV1`
- **NEW_PROPOSED:** `QuarantinePolicyV1`
- **NEW_PROPOSED:** `ReconciliationJournal interface if persistence is required`
...
- `CD-P13-01` — classify crash/omission/duplication/reordering/partition/malicious evidence
- `CD-P13-02` — define eventual settlement/containment/reclaim policies
- `CD-P13-03` — add bounded retry/terminal escalation
- `CD-P13-04` — cold-restart reconciliation design without authority resurrection
...
- TLA+ liveness under declared fault assumptions.
