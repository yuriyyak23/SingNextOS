# P00 — Live baseline, source/package freeze and executable inventory

## 1. Goal
Live baseline, source/package freeze and executable inventory.

## 2. Rationale / live gap
- source/package drift can invalidate every downstream claim.
- same SemVer must not imply same bytes.
...
- **VERIFIED_EXISTING:** `global.json`
- **VERIFIED_EXISTING:** `contracts/SingPlus.Contracts/SingPlus.Contracts.csproj`
- **VERIFIED_EXISTING:** `src/Runtime/SingPlus.Runtime/SingPlus.Runtime.csproj`
- **VERIFIED_EXISTING:** `HybridCPU_ExternalRuntime.Contracts/HybridCPU_ExternalRuntime.Contracts.csproj`

## 6. NEW_PROPOSED surfaces
- **NEW_PROPOSED:** `QualificationTupleV2`
- **NEW_PROPOSED:** `semantic co-design baseline manifest`
...
- none beyond reproducible hash/API checks.
