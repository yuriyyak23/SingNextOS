# P04 — Implement SingNext OperationObligations as non-authoritative semantic snapshot

## 1. Goal
Implement SingNext OperationObligations as non-authoritative semantic snapshot.

## 2. Rationale / live gap
- no current general OperationObligations symbol.
- must derive from current owners without minting new authority.
...
- **VERIFIED_EXISTING:** `src/Runtime/SingPlus.Runtime/Services/EndpointSessionInvocationRegistry.cs`

## 6. NEW_PROPOSED surfaces
- **NEW_PROPOSED:** `OperationObligationsV1`
- **NEW_PROPOSED:** `OperationObligationSnapshot`
- **NEW_PROPOSED:** `OperationObligationComposer`
...
- property tests for snapshot exactness/non-authority.
