# P15 — Close TCB/bootstrap/durable authority follow-ups and decide IFC scope

## 1. Goal
Close TCB/bootstrap/durable authority follow-ups and decide IFC scope.

## 2. Rationale / live gap
- ManagedCap has a single AdmissionVerifier; root/durable authority must not create new capability universes.
- checkpoint must restore fresh authority.
...
- **VERIFIED_EXISTING:** `tools/SingPlus.Admission/AdmissionVerifier.cs`
...
- **NEW_PROPOSED:** `ManagedExecutionContractV1`
- **NEW_PROPOSED:** `BootAuthorityPolicy document/contract`
- **NEW_PROPOSED:** `DurableAuthorityReconstructionRequest`
- **NEW_PROPOSED:** `IFC decision record`
...
- TCB assumptions table; no theorem prover until stable.
