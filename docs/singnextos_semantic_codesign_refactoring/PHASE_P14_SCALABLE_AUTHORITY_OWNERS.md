# P14 — Scale logical owners without replicated authority truth

## 1. Goal
Scale logical owners without replicated authority truth.

## 2. Rationale / live gap
- current owners use local locks in important paths.
- manycore scale must not reinterpret one owner as one global lock.
...
- **NEW_PROPOSED:** `OwnerShardKey policies`
- **NEW_PROPOSED:** `BudgetEscrowPartition`
- **NEW_PROPOSED:** `AuthorityPerformanceCounters`
...
- conservation/shard linearizability model where escrow is introduced.
