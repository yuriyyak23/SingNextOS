# P06 — Compose authority, refinement, provider admission and CPU legality before submit

## 1. Goal
Compose authority, refinement, provider admission and CPU legality before submit.

## 2. Rationale / live gap
- existing ResourceAdmission and IComputeSubmissionLegalityGate are the correct seams.
- new sentry must not become authority.
...
- **VERIFIED_EXISTING:** `src/Runtime/SingPlus.Runtime/VNext/ComputePlanResourceAdmission.cs`
...
- **NEW_PROPOSED:** `SemanticExecutionAdmissionDecision`
- **NEW_PROPOSED:** `CoDesignRejectKind`
...
- TLA+/PlusCal or bounded state exploration of prepare/revalidate/submit.
