...
| 7 | [`07_COMPUTE_ACCELERATORS.md`](07_COMPUTE_ACCELERATORS.md) | **In progress:** bounded DSC1 Copy v1 Host model, exact completion/wakeup and conservative DMA↔DSC1 mapping-use interlock; executable HybridCPU remains blocked. |
...
| mapping/remap | host-backed exact owned-region mapping plus conservative bridge-private DMA↔DSC1 active-use exclusion for one complete mapping identity | translation/invalidation mechanisms; no generic atomic ownership remap or cross-engine coherence proof | Phases 4, 7 |
...
| DMA | bounded local v5 submit/completion/visibility/closure model plus generation-bound completion notification and conservative exclusion from simultaneous DSC1 use of the same exact mapping; no executable HybridCPU DMA | neutral admission-only grant plus grant-scoped prepare/acquire visibility evidence merged on master; no neutral submit/completion/cancel or cross-engine interlock API | Phases 5–7 |
...
| accelerators | bounded DSC1 Copy v1 Host `ModelOnly` lifecycle with private staging and exact terminal wakeup, plus a local interlock proven with a faithful combined DMA↔DSC1 test provider; no Host executable DMA, and HybridCPU fails closed | MatrixTile, DSC1 and scoped L7 paths are code-confirmed internally, but no stable neutral compute or cross-engine mapping-use facade is supplied | Phase 7 in progress; `EXT-HCPU-004`/`005` |
| coherence/fences | explicit model DMA prepare/acquire boundaries and conservative DMA↔DSC1 whole-mapping exclusion; no CPU-alias mutation epoch or global-coherence assumption | explicit non-coherent fence requirements exist; cross-engine and universal coherence are not proven | Phases 1, 4, 5, 7; `EXT-HCPU-004`/`005` |
