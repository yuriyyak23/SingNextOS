...
The strict ordered re-audit at qualification HEAD `6227ea7cf258ef6ffce52001d4d2ffee07355b35` re-read the async normative requirements, live generated synchronous sentries, suspended-state verifier, ordinary cancellation owner tests, gate table, tuple and claim matrix while preserving the complete dirty worktree. The current containment/cancellation/direct-contour/default-gate/closure lane passed 30/30. The immediately preceding exact full non-GUI run on the same rebuilt executable tree recorded 1350 passed, the same 8 unrelated failures, and 2 skipped in `SingPlus.Tests`; Runtime and solution builds were 0-warning/0-error. No additional implementation defect was confirmed, and no async runtime or NativeAOT claim was added.
...
FutureGated owner: SIP generator plus runtime sentry/invocation owners. Missing prerequisite/evidence: generated async sentry continuation ABI; proof that service state machines never enter Job state; heap-safe lease/use lifetime across suspension; cancel-before-start, pre-suspend, suspended, completion race, session close/restart, Region reclaim, provider completion-versus-visibility tests; and exact runtime-mode qualification. Bypassing that ABI would require the Job runtime to retain a service-created Task/ValueTask/awaiter/state machine or lose owner-controlled cancellation and lease settlement, directly violating the ManagedCap boundary. NativeAOT remains independently FutureGated because there is no NativeAOT build/linkage/state-machine artifact or execution evidence; it cannot inherit the Windows x64 JIT result.
