...
public sealed class PlatformAuthorityBoundaryTests
{
    ...
    [Fact]
    [Trait("Category", "Runtime")]
    public void VirtualizationSipSurfaceContainsNoProviderOrHardwareAuthorityVocabulary()
    {
        var forbiddenTerms = new[] { "PlatformProvider", "HybridCpu", "VMCS", "VMX", "PhysicalAddress", "Iommu", "PageTableRoot" };
        var surface = typeof(IVirtualizationService).Assembly
            .GetExportedTypes()
            .Where(static type => type.Namespace == "SingPlus.Sip.Virtualization")
            .SelectMany(DescribePublicSurface)
            .ToArray();

        foreach (var forbidden in forbiddenTerms)
            Assert.DoesNotContain(surface, entry => entry.Contains(forbidden, StringComparison.OrdinalIgnoreCase));
    }
    ...
    public void CorePlatformPublicApiDoesNotExposeRawHardwareOrCompatibilityState()
    {
        ...
        Assert.Equal(PlatformFeatureAvailability.Unavailable,
            PlatformFeatureManifest.Empty.Resolve(PlatformFeatureFamily.VmxCompatibility).Availability);
    }
